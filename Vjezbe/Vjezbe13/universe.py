...
class Planet:
    def __init__(self, m, x0, y0, v0, radius):
        self.m = m
        self.v0 = v0
        self.x0 = x0
        self.y0 = y0
...
class Svemir:
    ...
    def evolve(self, vrijeme, dt):
        ...
        while self.t < vrijeme:
            self.__move()
'''
    def sudar(self, komet, dt):
        self.dt = dt
        self.t = 0
        while self.t < 2*godina:
            self.__move()
            for planet in self.planets:
                if planet != komet:
                    if np.sqrt((planet.x[-1]-komet.x[-1])**2 + (planet.y[-1]-komet.y[-1])**2) < (planet.radius+komet.radius):
                        x0 = komet.x0
                        y0 = komet.y0
                        print("Pogodak", x0, y0)
                        break
'''
class Komet:
    def __init__(self, m, x0, y0, v0, theta, radius):
        self.m = m
        self.v0 = v0
        self.x0 = x0
        self.y0 = y0
